    ...
    <div>
        <div class="weui-list_access">
            ...
            <template v-if="type==='2'">
                <div v-if="header" class="weui-t_bar">{{ header }}{{ list.length ? '（'+list.length+'）' : '' }}</div>
                <a class="weui-media_box" :class="{ 'weui-media_box_mt' : hasMt }" v-for="item in list" :href="item.url">
            ...
            <template v-if="type==='4'">
                <div v-if="header" class="weui-t_bar">{{ header }}{{ list.length ? '（'+list.length+'）' : '' }}</div>
                <a class="weui-media_box weui-media_box_mt" v-for="item in list" :href="item.url">
                    <div class="weui-media-box__hd3">
                        <i class="iconfont" :class="item.icon"></i>
                    </div>
                    <div class="weui-media-box__bd">
...
.weui-media_box { position: relative; padding: 10px 15px; display: flex; align-items: center; color: #000; background: #fff; }
.weui-media_box_mt { margin-top: 15px; }
.weui-media_box_mt:before { content: ""; position: absolute; left: 0; top: 0; width: 100%; height: 1px; background: #ccc; transform: scale(1,.3); }
...
.weui-media-box__bd { flex: 1; min-width: 0; line-height: 1.4; }
